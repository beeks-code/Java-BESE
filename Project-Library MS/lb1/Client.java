...
public class Client {
    ...
    // ─────────────────────────────────────────────────────────────
    // ADMIN PORTAL — CRUD for Students only
    // ─────────────────────────────────────────────────────────────
    static void showAdminPortal() {
        ...
        JTextArea output = new JTextArea(6, 50);
        output.setEditable(false);
        ...
        // Rebuild form on dropdown change
        dropdown.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                // ══ CREATE ══
                if (choice.equals("Create Student")) {
                    JTextField sidF  = addField(formPanel, "Student ID (must not already exist):");
                    JTextField nameF = addField(formPanel, "Student Name:");
                    JTextField bidF  = addField(formPanel, "Book ID to issue — enter 1 to 10, or leave blank for none:");
                    addBookHint(formPanel);

                    JButton btn = styledButton("Create Student", new Color(34, 139, 34));
                    formPanel.add(btn);
                        ...
                        public void actionPerformed(ActionEvent e) {
                            ...
                            if (!bid.isEmpty()) {
                                try {
                                    ...
                                    if (b < 1 || b > 10) { output.setText("ERROR: Book ID must be between 1 and 10."); return; }
                                } catch (NumberFormatException ex) { output.setText("ERROR: Book ID must be a number."); return; }
                            }

                            output.setText(sendCommand("ADMIN_ADD|" + sid + "|" + name + "|" + (bid.isEmpty() ? "none" : bid)));
                        }
                    });
                }

                // ══ READ ══
                else if (choice.equals("Read Student")) {
                    JTextField sidF = addField(formPanel, "Student ID to view:");
                    JButton btn = styledButton("Read Student", new Color(70, 130, 180));
                    formPanel.add(btn);
                ...
                // ══ UPDATE ══
                    ...
                    JTextField sidF  = addField(formPanel, "Student ID (must already exist in database):");
                    JTextField nameF = addField(formPanel, "New Student Name:");
                    JTextField bidF  = addField(formPanel, "New Book ID — enter 1 to 10, or leave blank to remove book:");
                    addBookHint(formPanel);
                    ...
                    btn.addActionListener(new ActionListener() {
                        public void actionPerformed(ActionEvent e) {
                            ...
                            output.setText(sendCommand("ADMIN_UPDATE|" + sid + "|" + name + "|" + (bid.isEmpty() ? "none" : bid)));
                        }
                    });
                }
    ...
    static JTextField addField(JPanel panel, String labelText) {
        ...
        field.setMaximumSize(new Dimension(350, 28));
    ...
    static void addBookHint(JPanel panel) {
        JLabel hint = new JLabel("<html>&nbsp;&nbsp;<b>Book IDs:</b> 1=Java &nbsp;2=Data Structures &nbsp;3=OS &nbsp;4=DBMS &nbsp;5=Networks<br>" +
        ...
        hint.setBorder(BorderFactory.createEmptyBorder(4, 8, 8, 8));
